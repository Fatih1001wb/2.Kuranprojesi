    ...
    <div className="ayet-kart">
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        ...
        <button
          ...
          onClick={() => sesOynat(ayetSesiUrl(sureNo, arapca.numberInSurah), `${sureAd} · ${arapca.numberInSurah}. ayet`)}
        >
          ▶ Dinle
        </button>
      </div>
      ...
      {tefsirAcik && tefsir && (
        <div style={{ marginTop: 8 }}>
          <button onClick={() => setTefsirGoster((s) => !s)} style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            {tefsirGoster ? 'Tefsiri Gizle' : 'Tefsiri Göster'}
          </button>
          {tefsirGoster && (
            <div
              style={{
                marginTop: 6,
                fontSize: yaziBoyutu - 2,
                lineHeight: 1.8,
                color: 'var(--text-sec)',
                padding: '14px 16px',
                borderRadius: 8,
...
export default function AnaSayfa({ aktifSure, geriDon, kayitlar, auth, karanlik, onOkumayaBasla }) {
  ...
  useEffect(() => {
    ...
    sureyiGetir(aktifSure.number)
      .then(([ar, tr]) => {
        if (ar.status !== 'OK' || tr.status !== 'OK') {
          throw new Error('SURE_YUKLENEMEDI');
        }
  ...
  const sureyiDinle = () => {
    if (!aktifSure) return;
    setSes({ url: sureSesiUrl(aktifSure.number), etiket: `${sureAdi} · Tam sure` });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', position: 'relative', zIndex: 1 }}>
      {okumaModuAcik && arapcaAyetler.length > 0 && (
        <OkumaModu
          ayetler={arapcaAyetler}
          meal={turkceMeal}
          sureAd={sureAdi}
          nuzulYeri={nuzulYeri}
          yaziBoyutu={yaziBoyutu}
          karanlik={karanlik}
          onKapat={() => setOkumaModuAcik(false)}
        />
      )}

      {aktifSure && (
        <div className="sure-topbar">
          <button className="btn-altin" onClick={geriDon}>
            ← Geri
          </button>

          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="sure-baslik-buyuk" style={{ fontFamily: "'Cinzel',serif", fontSize: 18 }}>
              {sureAdi}
            </div>
            <div className="sure-baslik-kucuk" style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              {nuzulYeri} · {aktifSure.numberOfAyahs} ayet
            </div>
          </div>

          <button className="btn-altin" onClick={sureyiDinle}>
            🎧 Dinle
          </button>
          <button className="btn-altin" onClick={tefsirToggle} disabled={tefsirYukleniyor}>
            {tefsirYukleniyor ? 'Yükleniyor...' : '📚 Tefsir'}
          </button>
          <button className="btn-altin" onClick={() => setOkumaModuAcik(true)} disabled={arapcaAyetler.length === 0}>
            ⛶ Odak
          </button>

          <div className="topbar-yazi-kontrol" style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <button className="btn-altin" style={{ width: 38, justifyContent: 'center', padding: '6px 0' }} onClick={() => setYaziBoyutu((b) => Math.max(14, b - 2))}>
              A-
            </button>
            <span style={{ fontSize: 13, color: 'var(--text-muted)', minWidth: 20, textAlign: 'center' }}>{yaziBoyutu}</span>
            <button className="btn-altin" style={{ width: 38, justifyContent: 'center', padding: '6px 0' }} onClick={() => setYaziBoyutu((b) => Math.min(32, b + 2))}>
              A+
            </button>
          </div>
          ...
          {girisliMi && (
            <button
              ...
              onClick={() =>
                yerIsaretli
                  ? kayitlar.yerIsaretiKaldir(aktifSure.number)
                  : kayitlar.yerIsaretiEkle({ number: aktifSure.number, ad: sureAdi })
              }
            >
              {yerIsaretli ? '🔖 İşaretli' : '🔖 Yer İşareti'}
            </button>
          )}
          ...
          {girisliMi && (
            <button
              ...
              onClick={() =>
                tamamlandi
                  ? kayitlar.okunduKaldir(aktifSure.number)
                  : kayitlar.okunduEkle({ number: aktifSure.number, ad: sureAdi })
              }
            >
      ...
      <div ref={icerikRef} className="icerik-alani" style={{ paddingBottom: ses ? 'calc(var(--player-h) + 20px)' : 0 }}>
        ...
        {yukleniyor && (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-muted)' }}>
            <div style={{ fontFamily: "'Cinzel',serif", fontSize: 14, letterSpacing: 4, marginBottom: 16 }}>YÜKLENİYOR</div>
        ...
        {!yukleniyor && !hata && arapcaAyetler.length > 0 && (
          <div className="icerik-ic">
            <div style={{ textAlign: 'center', padding: '28px 0 32px', borderBottom: '2px solid var(--gold-border)', marginBottom: 8 }}>
              ...
              <div style={{ fontSize: 'clamp(13px,3vw,16px)', color: 'var(--text-muted)' }}>
                {nuzulYeri} suresi · {aktifSure.numberOfAyahs} ayet
              </div>
              <div style={{ width: 55, height: 2, background: 'linear-gradient(90deg,transparent,var(--gold),transparent)', margin: '12px auto 0' }} />
            </div>
